...
BUBBLE_TEXT = 'ⒶⒷⒸⒹⒺⒻⒼⒽⒾⒿⓀⓁⓂⓃⓄⓅⓆⓇⓈⓉⓊⓋⓌⓍⓎⓏⓐⓑⓒⓓⓔⓕⓖⓗⓘⓙⓚⓛⓜⓝⓞⓟⓠⓡⓢⓣⓤⓥⓦⓧⓨⓩ ⓪①②③④⑤⑥⑦⑧⑨'
PLAIN_TEXT = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz 0123456789'
...
CARDS_CHEVALIER = ('🂬', '🂼', '🃌', '🃜')  # Chevalier/Knight
...
GREEK_ACCENT_UPPER = (
    'Ἀ', 'Ἁ', 'Ἂ', 'Ἃ', 'Ἄ', 'Ἅ', 'Ἆ', 'Ἇ',
    'ᾈ', 'ᾉ', 'ᾊ', 'ᾋ', 'ᾌ', 'ᾍ', 'ᾎ', 'ᾏ', 'Ᾰ', 'Ᾱ', 'Ὰ', 'Ά', 'ᾼ',
    'Ἐ', 'Ἑ', 'Ἒ', 'Ἓ', 'Ἔ', 'Ἕ', 'Ὲ', 'Έ',
    'Ἠ', 'Ἡ', 'Ἢ', 'Ἣ', 'Ἤ', 'Ἥ', 'Ἦ', 'Ἧ',
    'ᾘ', 'ᾙ', 'ᾚ', 'ᾛ', 'ᾜ', 'ᾝ', 'ᾞ', 'ᾟ', 'Ὴ', 'Ή', 'ῌ',
    'Ἰ', 'Ἱ', 'Ἲ', 'Ἳ', 'Ἴ', 'Ἵ', 'Ἶ', 'Ἷ', 'Ῐ', 'Ῑ', 'Ὶ', 'Ί',
    'Ὀ', 'Ὁ', 'Ὂ', 'Ὃ', 'Ὄ', 'Ὅ', 'Ὸ', 'Ό',
    'Ῥ',
    'Ὑ', 'Ὓ', 'Ὕ', 'Ὗ', 'Ῠ', 'Ῡ', 'Ὺ', 'Ύ',
    'Ὠ', 'Ὡ', 'Ὢ', 'Ὣ', 'Ὤ', 'Ὥ', 'Ὦ', 'Ὧ',
    'ᾨ', 'ᾩ', 'ᾪ', 'ᾫ', 'ᾬ', 'ᾭ', 'ᾮ', 'ᾯ', 'Ὼ', 'Ώ', 'ῼ',
)
...
GREEK_ACCENT_LOWER = (
    ...
    'ὐ', 'ὑ', 'ὒ', 'ὓ', 'ὔ', 'ὕ', 'ὖ', 'ὗ',
    'ὺ', 'ύ', 'ῠ', 'ῡ', 'ῢ', 'ΰ', 'ῦ', 'ῧ',
    'ὠ', 'ὡ', 'ὢ', 'ὣ', 'ὤ', 'ὥ', 'ὦ', 'ὧ', 'ὼ', 'ώ', 'ῶ',
    'ᾠ', 'ᾡ', 'ᾢ', 'ᾣ', 'ᾤ', 'ᾥ', 'ᾦ', 'ᾧ', 'ῲ', 'ῳ', 'ῴ', 'ῷ'
)
...
GREEK_ARCHAIC = ('Ϝ', 'Ϛ', 'Ϟ', 'Ϙ', 'Ϡ', 'ϝ', 'ϛ', 'ϟ', 'ϙ', 'ϡ', 'ϳ', 'ϗ', '϶')
...
def replacecurlycommas(_str: str) -> str:
    ...
    'This is, a test.'
    """
    return _str.replace('،', ',').replace('、', ',').replace('،', ',').replace('◌̦', ',').replace('︐', ',').replace('︑', ',').replace('᠂', ',').replace('‍̓', ',').replace('‍፣', ',').replace('‍꛵', ',')
...
def rmcurlyquotes(_str: str) -> str:
    ...
    'This is a test.'
    """
    return _str.replace('“', '').replace('”', '').replace('‘', '').replace('’', '').replace('′', '').replace('″', '').replace('‴', '').replace('″', '')
...
def rmspecialwhitespace(_str: str) -> str:
    ...
    'This is a test.'
    """
    return _str.replace('\u00A0', '').replace('\u1680', '').replace('\u16EB', '').replace('\u16EC', '').replace('\u180E', '').replace('\u2000', '').replace('\u2001', '').replace('\u2002', '').replace('\u2003', '').replace('\u2004', '').replace('\u2005', '').replace('\u2006', '').replace('\u2007', '').replace('\u2008', '').replace('\u2009', '').replace('\u200A', '').replace('\u200B', '').replace('\u202F', '').replace('\u205F', '').replace('\u3000', '').replace('\uFEFF', '')
...
def sqlstr(_obj, _strength: int=0) -> str:
    """Convert an object to a string and format the string
...
def fvalhtml(_search: str, _type: str) -> str:
    """Search a dict by value to find the paired value
...
def noescape(_hex: str) -> str:
    """Remove escapes from a single character hex string
    (\\U00000026, \\u0026, \\x26)
    """
    if '\\U' in _hex and len(_hex) is 10:
        _int = int(_hex.replace('\\U', ''), 16)
        _hex = hex(_int).replace('0x', '')
        ...
        if _hex.count('\\x') is 4:
            ...
            raise Exception('Invalid input passed to noescape()!')
...
def shrink_esc_utf16to8(_hex: str) -> str:
    r"""Shrink a 16-bit hex escape to a 8-bit hex escape
    (\\uhhhh) => (\\xhh)
    >>> shrink_esc_utf16to8('\\u002f')
    '\\x2f'
    """
    ...
    raise Exception(_hex + ' cannot be shrunk to a 8-bit hex escape!')


def shrink_esc_utf32to8(_hex: str) -> str:
    r"""Shrink a 32-bit hex escape to a 8-bit hex escape
    (\\Uhhhhhhhh) => (\\xhh)
...
def shrink_esc_utf32to16(_hex: str) -> str:
    r"""Shrink a 32-bit hex escape to a 16-bit hex escape
    (\\Uhhhhhhhh) => (\\uhhhh)
    ...
    raise Exception(_hex + ' cannot be shrunk to a 16-bit hex escape!')
...
def unescape(_str):
    ...
    'String with "special" characters'
    """
    _str = _str.replace('&#34;', '"').replace('&#39;', r'\'').replace('&#91;', '[')
    _str = _str.replace('&#40;', '(').replace('&#41;', ')').replace('&#93;', ']')
    return _str.replace('&#123;', '{').replace('&#125;', '}').replace('&#92;', r'\\')
...
def htmlunescape(_str: str):
    ...
    _str = _str.replace('&#x3c;', '<').replace('&#60;', '<').replace('&lt;', '<')
    _str = _str.replace('&#x3e;', '>').replace('&#62;', '>').replace('&gt;', '>')
    _str = _str.replace('&x26;', '&').replace('&#38;', '&').replace('&amp;', '&')
    return _str
...
def expandhexescape(_hex: str) -> str:
    r"""Convert a 16-bit hex escape to a 32-bit hex escape
    (\\uhhhh) => (\\Uhhhhhhhh)
    >>> expandhexescape('\\u202f')
    '\\U0000202f'
    """
    ...
    raise Exception(_hex + ' cannot be converted to a 32-bit hex escape!')
...
def int2char(_int, _numbase: str='10', _escape: bool=True) -> str:
    ...
    if isinstance(_int, int):
        if _escape:
            ...
            raise ValueError(
                'An out-of-range integer passed to int2char()!'
            )
    raise TypeError('Invalid datatype passed to int2char()!')
...
def refnum2char(_refnum: str, _escape: bool=True) -> str:
    ...
    if 'x' not in _refnum:
        _refnum = int(_refnum.replace('&#', '').replace(';', ''), 10)
        rawhexstr = hex(_refnum).replace('0x', '')
    else:
        rawhexstr = _refnum.replace('&#x', '').replace(';', '')
    if _escape:
...
def refnum2int(_refnum: str) -> int:
    ...
    _refnum = _refnum.replace('&#', '').replace(';', '')
...
def utf8convchar2hexstr(_char: str) -> str:
    ...
    _char = _char.replace('\'', '').replace('\\x', '0x')
    return '{0}'.format(_char)
...
def char2hexstr(_char: str) -> str:
    ...
    if len(_char) is 1:
        ...
        for i in _char:
            _out = _out + hex(ord(i))
        return _out
    raise ValueError('Invalid data passed to char2hexstr()!')
...
def hex2unicodehex(_hex: str or int) -> str:
    ...
    if isinstance(_hex, str):
        ...
        if '\\x' in _hex:
            _hex = _hex.replace('\\x', '')
        if '\\U' in _hex:
            _hex = _hex.replace('\\U000', '').replace('\\U00', '')
            _hex = _hex.replace('\\U0', '').replace('\\U', '')
        if '\\u' in _hex:
            _hex = _hex.replace('\\u00', '').replace('\\u0', '')
            _hex = _hex.replace('\\u', '')
        if '\\0' in _hex:
            _hex = _hex.replace('\\0', '')
        if '&#x' in _hex:
            _hex = _hex.replace('&#x', '').replace(';', '')
        return 'U+{0}'.format(_hex)
    elif isinstance(_hex, int):
        _hex = hex(_hex).replace('0x', '')
        return 'U+{0}'.format(_hex)
    raise TypeError('Invalid datatype passed to hex2unicodehex()!')
...
def hex2cssnot(_hex: str or int) -> str:
    ...
    if isinstance(_hex, str):
        ...
        _hex = hex(_hex).replace('0x', '')
        return '\\0{0}'.format(_hex)
    raise TypeError('Invalid datatype passed to hex2cssnot()!')
...
def char2intstr(_char: str) -> int:
    ...
    if len(_char) is 1:
        ...
        for i in _char:
            _out += str(ord(i))
        return _out
    raise ValueError('Invalid data input in char2intstr()!')
...
def bubble2text(_str: str) -> str:
    ...
    """
    _str = _str.replace('\u202f', '').replace('\\u202f', '')
    return _str.translate(TRANS_TAB_BUBBLE2TEST).replace('\u20dd ', '').replace('\u20dd', '')
...
def square2text(_str: str) -> str:
    ...
    'Testing various characters: €, *, Ω, ᾲ, and ⛽.'
    """
    return _str.replace('\u202f', '').replace('\\u202f', '').replace('\u20de ', '').replace('\u20de', '').replace('  ', ' ')
